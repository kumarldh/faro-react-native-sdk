 ...
 * ```
 */
export function getRNInstrumentations(config = {}) {
    // Aligned with Flutter SDK FaroConfig defaults
    const { 
    // Error & crash tracking
    enableErrorReporting = true, enableCrashReporting = false, anrTracking = false, anrOptions = {}, 
    // Performance vitals
    cpuUsageVitals = true, memoryUsageVitals = true, refreshRateVitals = false, fetchVitalsInterval = 30000, frameMonitoringOptions = {}, 
    // Network
    ignoreUrls = [], enableHttpInstrumentation = {}, 
    // Console and user actions
    enableConsoleCapture = true, enableUserActions = true, 
    // Tracing
    enableTracing = false, tracingOptions = {}, } = config;
